...
ValueCallback *ValueCallback::findCallback(std::deque<ValueCallback *> &callbacks, const OIDType *const oid, bool walk, size_t startAt, size_t *foundAt)
{
    ...
    for (size_t i = startAt; i < callbacks.size(); i++)
    {
        ...
        // __DF("%s == %s", oidVal, callback->OID->str());
...
{
#if DEBUG == 5
    __DF("getting value for callback of OID: %s, type: %d\n", callback->OID->string().c_str(), callback->type);
#endif
...
{
#if DEBUG == 5
    __DF("setting value for callback of OID: %s\n", callback->OID->string().c_str());
#endif
...
SNMP_ERROR_STATUS StringCallback::setTypeWithValue(BER_CONTAINER *rawValue)
{
    ...
    if (val->_value.length() >= this->max_len)
        return WRONG_LENGTH;
    this->setValue(val->_value.data(), this->max_len);
...
SNMP_ERROR_STATUS OpaqueCallback::setTypeWithValue(BER_CONTAINER *rawValue)
{
    ...
    if (val->_dataLength > this->data_len)
        return WRONG_LENGTH;
    this->setValue(val->_value, this->data_len);
...
{ // returns true if oid1 EARLIER than oid2
    ...
    {
        if (map1[j] != map2[j])
        { // if they're the same then we're on same level
