...
#define ASSERT_ASN_TYPE_AT_STATE(value, TYPE, STATE)                  \
    ...
    {                                                                 \
        __DF("expecting value to be " STR(TYPE) " for " #STATE "\n"); \
...
#define ASSERT_ASN_STATE_TYPE(value, STATE)                                                              \
    ...
    {                                                                                                    \
        __DF("expecting " STR(ASN_TYPE_FOR_STATE_##STATE) " for " #STATE " failed: %d\n", value->_type); \
...
#define ASSERT_ASN_PARSING_TYPE_RANGE(value, LOW_TYPE, HIGH_TYPE)           \
    ...
    {                                                                       \
        __DF("expecting vartype for PDU failed: %d\n", value->_type);       \
        return SNMP_PARSE_ERROR_GENERIC;                                    \
    }
...
SNMP_PACKET_PARSE_ERROR SNMPPacket::parsePacket(ComplexType *structure, enum SNMPParsingState state)
{
    for (const auto &value : structure->values)
    {
        ...
        case VARBIND:
        {
            ...
            {
#if DEBUG == 5
                __DF("expecting VARBIND TO CONTAIN 2 OBEJCTS; %u\n",
...
{
#if DEBUG == 5
    __DF("- parsing %d bytes\n", max_len);
#endif
    if (buf[0] != 0x30)
    {
        __DL("(!) first byte error");
    ...
    if (decodePacket <= 0)
    {
        __DL("(!) failed to decode fromBuffer");
        return decodePacket;
    }
...
{
#if DEBUG == 5
    __DL("generateVarBindList from SNMPPacket");
#endif
